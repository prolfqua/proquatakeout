...
xx <- proquatakeout::mark_decorelated(qvalFiltV, config, minCorrelation = 0.5 )
...
```{r rankprec}
qvalFiltImputed <- proquatakeout::impute_correlationBased(qvalFiltCorr, config)
qvalFiltImputed <- prolfqua::rank_peptide_by_intensity(qvalFiltImputed, config)
...
mean_na <- function(x, name = FALSE){
  ...
  mean(x,na.rm = TRUE)}
proteinIntensities <- prolfqua::aggregate_intensity_topN(qvalFiltImputed,
                                               config,
                                               .func = mean_na,
                                               N = 3)$data
